...
export function TabCard({ tab }: TabCardProps) {
    ...
    return (
        <div
            className="group flex items-center gap-3 px-3 py-2.5 cursor-pointer transition-colors hover:bg-white/[0.03]"
            onClick={handleClick}
            role="button"
            tabIndex={0}
        >
            {/* Favicon */}
            <div className="w-5 h-5 rounded flex-shrink-0 flex items-center justify-center overflow-hidden" style={{ background: 'var(--bg-elevated)' }}>
                {faviconUrl ? (
                    <img
                        src={faviconUrl}
                        alt=""
